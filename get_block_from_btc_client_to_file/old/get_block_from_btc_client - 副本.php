...
for($i=0; $i<30000; $i++)
{
	...
	//将block插入数据库
	$query = "INSERT INTO `block` (`height`, `hash`,`json_content`) VALUES ('$now', '$getnowblockhash', '$getnowblock_json')";
	//echo $query;
	...
	foreach($getnowblock['tx'] as $value)
	{
		...
		if($result === false)
		{
			...
			file_put_contents("log.txt", $txid.':'.$getnowblockhash."\r\n", FILE_APPEND);
		}
	}
	
	
	//显示当前进度
	echo "$now:$getnowblockhash\r\n";
	echo "\r\n";
