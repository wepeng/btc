...
for($i=0; $i<50000; $i++)
{
	...
	//将block插入文件夹
	$flag = file_put_contents("D:/1/".$getnowblock['height'].".txt", $getnowblock_json);
	if($flag === false)
	{
	    echo("ERROR in file_put_contents :"."D:/1/".$getnowblock['height'].".txt"); 
		exit;
	}
	
	//将block插入数据库
	$query = "INSERT INTO `block` (`height`, `hash`) VALUES ('".$getnowblock['height']."', '$getnowblockhash')";
	//echo $query;
